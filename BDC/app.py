...
# Gemini API setup
GOOGLE_API_KEY = "AIzaSyCBt1P_Hr9RK4-P6e872pjbUoXCObGlO6U"  # Replace with your API key
genai.configure(api_key=GOOGLE_API_KEY)

def extract_with_gemini(file_path, file_name, category):
    """Extract metadata from file using Gemini API"""
    try:
        ...
        # For image files, we can directly send to Gemini
        if file_type in ['jpg', 'jpeg', 'png', 'gif']:
            ...
            content_parts = [
                ...
                {"inline_data": {"mime_type": f"image/{file_type}", "data": base64.b64encode(image_data).decode('utf-8')}}
            ]
            
            response = model.generate_content(content_parts)
            
        # For PDFs, we could either:
        # 1. Extract first few pages as images and send those
        # 2. Extract text with PyPDF2 and send text
        # Let's use option 2 for this example
        elif file_type == 'pdf':
            import PyPDF2
            content = ""
            try:
            ...
            prompt = f"""
            Analyze this document content (filename: {file_name}, category: {category}) and extract key metadata.
        ...
        # For text files
        elif file_type in ['txt', 'md', 'csv', 'html', 'js', 'py', 'json']:
        ...
        # For unknown file types, just use the filename
        else:
            prompt = f"""
            Based on the filename '{file_name}' and category '{category}', 
            create metadata for what appears to be a {category} document.
        ...
        try:
            ...
            for field in ["summary", "keywords", "main_themes", "document_type", "dates_mentioned", "entities"]:
                if field not in result:
                    if field in ["keywords", "main_themes", "dates_mentioned", "entities"]:
                        result[field] = []
                    else:
                        result[field] = ""
            return result
            ...
            return {
                "summary": f"Extracted content summary (JSON parsing failed: {str(e)})",
                "keywords": [],
                "main_themes": [],
                "document_type": f"{category} document",
                "dates_mentioned": [],
                "entities": []
            }
        ...
        # Return structured data even in case of error
        return {
            "summary": "Could not extract metadata due to an error: " + str(e),
            "keywords": [],
            "main_themes": [],
            "document_type": f"{category} document",
            "dates_mentioned": [],
...
def extract_metadata():
    try:
        ...
        if not file_data or not file_name or not category:
            return jsonify({
                'status': 'error',
                'message': 'Missing required data'
            }), 400
            
        # Log the request (without the actual file data)
        print(f"Processing metadata extraction for: {file_name}, Category: {category}")
        
        try:
            # Extract the base64 content (remove the data:application/pdf;base64, part)
            # Handle different file types
            if ';base64,' in file_data:
                encoded_data = file_data.split(';base64,')[1]
            else:
            ...
            # Save temporarily to analyze with Gemini
            ...
            print(f"File saved to {temp_file_path}, extracting metadata...")
            metadata = extract_with_gemini(temp_file_path, file_name, category)
            ...
            if 'summary' not in metadata:
                metadata['summary'] = "Could not extract summary"
            if 'keywords' not in metadata:
                metadata['keywords'] = []
            if 'main_themes' not in metadata:
                metadata['main_themes'] = []
            if 'document_type' not in metadata:
                metadata['document_type'] = f"{category} document"
            if 'dates_mentioned' not in metadata:
                metadata['dates_mentioned'] = []
            if 'entities' not in metadata:
                metadata['entities'] = []
            
            # Save metadata to database
            save_result = save_metadata_to_db(file_name, category, metadata)
            if not save_result:
                return jsonify({
                    'status': 'error',
            ...
            # Still try to save some basic metadata
            basic_metadata = {
                'summary': f"Could not extract metadata due to processing error: {str(e)}",
                'keywords': [],
                'main_themes': [],
                'document_type': f"{category} document",
                'dates_mentioned': [],
            ...
            save_metadata_to_db(file_name, category, basic_metadata)
...
def save_metadata_to_db(filename, category, metadata_json):
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO metadata_record (filename, category, timestamp, metdata)
            VALUES (?, ?, ?, ?)
        """, (filename, category, datetime.now().strftime('%Y-%m-%d %H:%M:%S'), metadata_str))
